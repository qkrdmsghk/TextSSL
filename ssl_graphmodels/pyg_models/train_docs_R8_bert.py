...
def bert_transform(sent):
    ...
    encoded_layer, _ = model(torch.tensor([tokens_ids]))
...
if __name__ == '__main__':
    ...
    # SEEDS = [2222, 239874, 456, 2222, 239874, 456]
    # SEEDS = [1944, 789, 123, 456]
    SEEDS = [456, 1, 444, 747, 1000]
    ...
    loader = LoadDocsData(name=name, type=args.type, pretrained='bert')
    ...
    for SEED in SEEDS:
        for i, methods in enumerate(tqdm(METHODS)):
            ...
            # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=int(args.lr_dc_step), gamma=float(args.lr_dc))
