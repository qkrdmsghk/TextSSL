...
#
# class LoadData():
#     def __init__(self, name, type, dic_path, memo=''):
#         self.name = name
#         self.type = type
...
#         self.test_set = []
#
#         self.dictionary = Word2Vec.load(os.path.join('/data/project/yinhuapark/DATA_PRE', name, type, dic_path))
#
#     class Handle_data(object):
#         def __call__(self, data):
#             data.x_n_id = data.x_n
#             data.x_p_id = data.x_p
#
#             data.x_n = torch.where(torch.isnan(data.x_n), torch.full_like(data.x_n, 0), data.x_n)
#             data.x_p = torch.where(torch.isnan(data.x_p), torch.full_like(data.x_p, 0), data.x_p)
#             data.y_p = data.y_p[:, 1]
#             # print('x_n!!!')
#             # print(data.x_n[data.x_n!=data.x_n].size(0))
#             # print('x_p')
#             # print(data.x_p[data.x_p!=data.x_p].size(0))
#             return data
#
#     def split_train_val_data(self, seed):
#         pos_id = []
#         neg_id = []
#         print(self.train_set)
#         np.random.seed(seed)
#         for id, data in enumerate(self.train_set):
#             if data.y_p == torch.tensor([1]):
#                 pos_id.append(id)
#             else:
#                 neg_id.append(id)
#
#         test_pos_num = 0
#         test_neg_num = 0
#         for id, data in enumerate(self.test_set):
#             if data.y_p == torch.tensor([1]):
#                 test_pos_num += 1
#             else:
#                 test_neg_num += 1
#
#         val_id = random.sample(pos_id, test_pos_num) + random.sample(neg_id, test_neg_num)
#         train_id = [x for x in range(len(self.train_set)) if x not in val_id]
#         self.val_set = self.train_set[val_id]
#         self.train_set = self.train_set[train_id]
#         return self.train_set, self.val_set
#
#
#     def get_train_test(self, batch_size, seed):
#         self.test_set = PNGD(name=self.name, split=self.memo+'test', type=self.type, dic_path=self.dic_path, memo=self.memo, transform=self.Handle_data())
#         self.train_set = PNGD(name=self.name, split=self.memo+'train', type=self.type, dic_path= self.dic_path, memo=self.memo, transform=self.Handle_data())
#         self.train_set, self.val_set = self.split_train_val_data(seed)
#         #
#         label_distribution(self.train_set)
#         label_distribution(self.test_set)
#         label_distribution(self.val_set)
#
#         # loader = DataLoader(test_set[:], batch_size=1)
#
#         train_loader = DataLoader(self.train_set[:], batch_size=batch_size, follow_batch=['x_n', 'x_p'], shuffle=True)
#         val_loader = DataLoader(self.val_set[:], batch_size=batch_size, follow_batch=['x_n', 'x_p'], shuffle=False)
#         test_loader = DataLoader(self.test_set[:], batch_size=batch_size, follow_batch=['x_n', 'x_p'], shuffle=False)
#
#         return train_loader, val_loader, test_loader
#
#     def get_case(self, id=None):
#         self.case_set = PNGD(name=self.name, split='test', type=self.type, dic_path=self.dic_path, memo=self.memo, transform=self.Handle_data())
#         if id == None:
#             case_loader = DataLoader(self.case_set[:], batch_size=1, follow_batch=['x_n', 'x_p'], shuffle=False)
#         else:
#             case_loader = DataLoader(self.case_set[id:id+1], batch_size=1, follow_batch=['x_n', 'x_p'], shuffle=False)
#         return case_loader
