...
def test(loader):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            ...
            nb_total += len(y)
            labels.append(y)
            preds.append(pred.data.argmax(dim=-1))
...
if __name__ == '__main__':
    ...
    for SEED in SEEDS:
        for i, methods in enumerate(tqdm(METHODS)):
            ...
            model_name = get_model_name(num_layer, args.aggregate, SEED, args.tr_split)
            print('---------------------------------------------------------------------------')
            print('---------------------------------------------------------------------------')
            ...
            best_model, best_epoch, test_results, all_results, best_preds, labels = train_main(train_loader, val_loader,
                                                                                               test_loader,
                                                                                               params['patience'])
            experiment.log_confusion_matrix(labels, best_preds, title='Test Confusion Matrix')
            print("Macro average Test Precision, Recall and F1-Score...")
            macro_p, macro_r, macro_f, _ = metrics.precision_recall_fscore_support(labels, best_preds, average='macro')
            print(metrics.precision_recall_fscore_support(labels, best_preds, average='macro'))

            print("\nMicro average Test Precision, Recall and F1-Score...\n")
            micro_p, micro_r, micro_f, _ = metrics.precision_recall_fscore_support(labels, best_preds, average='micro')
            print(metrics.precision_recall_fscore_support(labels, best_preds, average='micro'))
            ...
            result_df = pd.DataFrame([[params['num_layer'], params['aggregate'], best_epoch, test_results, macro_f]],
                                     columns=['num_layer', 'aggregation', 'converged_epoch', 'test_acc', 'macro_f'])
